...
BOOL CChartDlg::OnInitDialog() 
{
	...
	for (i = 0; i < m_pGraph->GetCurveCount (); i++)
	{
		...
		cItem.Format(_T("%d %s"), i + 1, m_pGraph->GetCurve(i).GetLabel());
	...
	for (i = 0; i < m_pGraph->GetXAxisCount (); i++)
	{
		...
		cItem.Format(_T("%d %s"), i + 1, m_pGraph->GetXAxis(i).GetLabel());
	...
	for (i = 0; i < m_pGraph->GetYAxisCount (); i++)
	{
		...
		cItem.Format(_T("%d %s"), i + 1, m_pGraph->GetYAxis(i).GetLabel());
