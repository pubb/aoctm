...
void CCurveDlg::PresetValues()
{
	if (m_pSerie)
	{
		...
		for (int i = 0; i < m_pSerie->GetGraph ()->GetCurveCount (); i++)
		{
			...
			{
				CString cItem;
				cItem.Format(_T("%d %s"), i + 1, m_pSerie->GetGraph ()->GetCurve(i).GetLabel());
...
void CCurveDlg::OnMavg() 
{
	...
	if (dlg.DoModal () == IDOK)
	{
		...
		ret.SetLabel(m_pSerie->GetLabel () + _T(" AVG, ") + cType);
		delete pTrend;
	}
}
...
void CCurveDlg::OnPolytrend()
{
	...
	for (int i = 0; i < m_pSerie->m_PS.m_nGlobalO; i++)
	{
		...
		CString cCoeff;
		cCoeff.Format(_T("%5.4e*X^%d"), m_pSerie->m_PS.m_fC[i], i);
	...
	ret.SetLabel(m_pSerie->GetLabel() + _T(" Trend (poly.) "));
...
void CCurveDlg::OnLintrend() 
{
	...
	ret.SetLabel(m_pSerie->GetLabel() + _T(" Trend (lin.)"));
}
...
void CCurveDlg::OnCubtrend() 
{
	...
	ret.SetLabel(m_pSerie->GetLabel() + _T(" Trend (cub.)"));
