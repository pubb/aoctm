...
void CXGraphAxis::DeleteColorRange(int nRange)
{
	...
	{
		//fred changed , unknown if it is ok
		m_ColorRanges.erase(m_ColorRanges.begin()+nRange);//&m_ColorRanges[nRange]);//m_ColorRanges.at(nRange));
	}
...
void CXGraphAxis::DeleteAxisMarker(int nMarker)
{
	if (nMarker < m_AxisMarkers.size())
		//fred changed 2007
		m_AxisMarkers.erase (m_AxisMarkers.begin()+nMarker);//&m_AxisMarkers[nMarker]);
...
void CXGraphAxis::Serialize( CArchive& archive )
{
    ...
    if( archive.IsStoring() )
    {
		...
		for ( i = 0; i < m_ColorRanges.size(); i++)
