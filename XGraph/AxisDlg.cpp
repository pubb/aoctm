...
void CAxisDlg::PresetValues()
{
	...
	if (!m_pAxis->GetDateTime())
	{
		...
		int nPos = m_pAxis->GetDisplayFmt().Find (_T("."),0);
		if (nPos >= 0)
		{
			CString cPrecision = m_pAxis->GetDisplayFmt().Mid (nPos+1);
			m_nPrecision = _ttoi(cPrecision.GetBuffer());
...
void CAxisDlg::OnApply() 
{
	...
	if (!m_bTimeAxis)
	{
		...
		if (fabs(fCurMin) > 1E6 || fabs(fCurMax) > 1E6 ||
			...
			cTmp.Format(_T("%%5.%de"), m_nPrecision);
		else
			cTmp.Format(_T("%%5.%df"), m_nPrecision);
